...
- 形式はWebP(透過あり)。長辺512px、遠景の山・街並み・島と、電車・車庫は長辺1024px(画面に大きく映るため)
- 縦横の比はコードで決まっている(アプリは画像をその比に引き伸ばして描く)。素材スキルの `assets.json` に同じ比を書き、取り込み時に余白で合わせる。コードの比を変えたら、素材スキルの `sync-check` でずれを確かめる
